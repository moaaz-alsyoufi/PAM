...
import { getEcommerceCustomerData } from "@/data/apps/ecommerce";
...
const CustomerRow = ({ customer, checkedAll }: { customer: IEcommerceCustomer; checkedAll: boolean }) => {
    const [checked, setChecked] = useState(checkedAll);
    const { id, name, date, image, email, gender, purchases, spend, verified, mobile_number } = customer;
    ...
    return (
        <>
            <TableRow className="cursor-pointer hover:bg-base-200/40" onClick={() => setChecked(!checked)}>
                ...
                <div>{purchases}</div>
                <div className="text-sm font-medium">${StringUtil.convertToCurrency(spend)}</div>
                {verified ? (
...
const CustomerTable = () => {
    ...
    const data = useMemo(() => getEcommerceCustomerData, []);
            ...
            <CardBody className={"p-0"}>
                ...
                <div className="overflow-auto">
                    <Table className="mt-2 rounded-box">
                        ...
                        <TableBody>
                            {data.slice(0, 11).map((customer, index) => (
                                <CustomerRow customer={customer} checkedAll={checkedAll} key={index} />
                            ))}
                        </TableBody>
                    </Table>
                </div>
